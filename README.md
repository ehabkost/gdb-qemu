...
To validate a `qemu-system-x86_64` binary in your system against reference
dumps, you can run:

    ./compat_check.py /usr/bin/qemu-system-x86_64 -O dump.json
    ./validate.sh dump.json

The first command will extract all info from the QEMU binary using QMP commands
and GDB.  The second command will compare the extracted dump against the
existing reference dumps in this repository.
...
Running `./compat_checker.py A B` checks if `B` can emulate `A` properly (for
example, limits like `max_cpus` can be larger on `B`).